...
def test_version_mismatch_fails_readme_changelog_and_page_consistency(
    ...
    (tmp_path / "README.md").write_text(
        "# Engineering Knowledge Base v0.0.7\n", encoding="utf-8"
    )
    (tmp_path / "CHANGELOG.md").write_text("## v0.0.7\n", encoding="utf-8")
    (tmp_path / "app.py").write_text("v0.0.7", encoding="utf-8")
    (tmp_path / "pages" / "1_test.py").write_text("v0.0.7", encoding="utf-8")

    result = version_consistency_check(
        tmp_path,
        app_version="0.0.7",
        app_title="工程知识库 v0.0.7",
    )

    assert result.status is CheckStatus.FAIL
    assert "README" in result.detail
    assert "CHANGELOG" in result.detail
